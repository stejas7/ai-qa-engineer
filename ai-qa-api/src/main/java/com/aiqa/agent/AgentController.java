...
public class AgentController {
    ...
    public AgentRun start(@RequestBody StartRequest request){
        if(request==null || request.agentType()==null || request.agentType().isBlank()) throw new IllegalArgumentException("agentType is required");
    ...
    public AgentStep addStep(@PathVariable UUID id,@RequestBody StepRequest request){return orchestrator.addStep(id,request.stepType(),request.input());}

    @PostMapping("/steps/{id}/complete")
    public AgentStep completeStep(@PathVariable UUID id,@RequestBody CompleteStepRequest request){return orchestrator.completeStep(id,request.output());}

    @PostMapping("/runs/{id}/complete")
    public AgentRun complete(@PathVariable UUID id,@RequestBody CompleteRunRequest request){return orchestrator.complete(id,request.summary());}
}
